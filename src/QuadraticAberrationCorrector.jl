...
function MakeCoefficientMatrix(ϕ)
    S = Matrix{eltype(ϕ)}(undef, length(ϕ), 6)
    S[:,1] .= 1
...
function MakeGridPhase(ϕ, ρx, ρy)
    ψ = Matrix{eltype(ϕ)}(undef, size(ϕ))
    for j in 1:ρx
        rⱼ = round(Int, (j - 1)size(ϕ, 2)/ρx + 1, RoundNearestTiesAway) : round(Int, j*size(ϕ, 2)/ρx, RoundNearestTiesAway)
        for i in 1:ρy
            rᵢ = round(Int, (i - 1)size(ϕ, 1)/ρy + 1, RoundNearestTiesAway) : round(Int, i*size(ϕ, 1)/ρy, RoundNearestTiesAway)
            ψ[rᵢ, rⱼ] .= MMASD(@view ϕ[rᵢ, rⱼ])
        end
    end
    return ψ
end
